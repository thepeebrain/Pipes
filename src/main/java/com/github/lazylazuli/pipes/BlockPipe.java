...
public class BlockPipe extends BlockContainer
{
	...
	public BlockPipe(String name, PropertyEnum<EnumFlow> flow)
	{
	...
	private IBlockState constructStateFromFlow(EnumFlow flow)
	{
		if (flow == null)
			flow = EnumFlow.DE;
		BlockPipe block = flow.ordinal() < 15 ? Pipes.PIPE : Pipes.PIPE_REVERSE;
	...
	{
		EnumFlow flow = state.getValue(((BlockPipe) state.getBlock()).flow);
	...
	{
		return new BlockStateContainer(this, Pipes.PIPE == null ? NORMAL : REVERSE);
	}
